"""
IC-4-M5 D1+D2: Cross-Layer Sycophancy + Answer Correctness Steering.

D1: Test sycophancy steering at layers 6, 10, 14, 18, 22.
    Measure inter/intra distances (SNR) and oracle/real gate effect at each layer.

D2: Train correctness probe on answerable questions.
    Test if steering can improve factual accuracy.

Key questions:
  - Which layer has the highest sycophancy SNR?
  - Does any layer show oracle_gap > 0?
  - Can correctness be steered?
"""
...
def _compute_vec_and_probe(model, tokenizer, samples, labels, layer):
    X, _ = _collect_prefill_features(model, tokenizer, samples, layer, "last_prompt_token")
    y = np.array([1 if l["label"] == "sycophantic" else 0 for l in labels])
    probe = _train_probe(X, y, cv_folds=3)
    return X, y, probe


def _run_gates(model, tokenizer, test_data, base_results, sv, layer, alpha, probe, probe_cfg):
    ...
    return oracle_rate, real_rate, n_non_syc, n_non_syc_real


def main():
    _log("=" * 56)
    _log("IC-4-M5 D1+D2: Cross-Layer + Correctness")
    _log("=" * 56)
    ...
    all_rows = []
    report_lines = ["IC-4-M5 D1+D2 REPORT", "=" * 56, ""]
    ...
    for layer in D1_LAYERS:
        ...
        for alpha in D1_ALPHAS:
            oracle_rate, real_rate, n_o, n_r = _run_gates(
    ...
    if min(len(correct_train), len(incorrect_train)) >= 3:
        ...
        for alpha in D2_ALPHAS:
            ...
            _log(f"  α={alpha:+.1f}: oracle_correct_rate={oracle_cr:.4f} ({n_c_oracle}/{len(oracle_d2_results)}) "
