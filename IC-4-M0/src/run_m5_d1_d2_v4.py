"""
IC-4-M5 D1+D2 v4: Reverse Alpha Scan (oracle only).
Key fix: D1 uses NEGATIVE alpha (push away from sycophantic),
         D2 uses POSITIVE alpha (push toward correct).

D1: oracle gate at layers 6,10,14,18,22 with alpha=-0.8,-1.0,-1.2.
D2: oracle gate at layer 12 with alpha=+0.6,+0.8,+1.0.
"""
...
REPORT_PATH = "results_m5/d1_d2_reverse_report.txt"
...
def main():
    _log("=" * 56)
    _log("IC-4-M5 D1+D2 v4: REVERSE ALPHA SCAN")
    _log("  D1: negative alpha [-0.8, -1.0, -1.2] (inhibit sycophancy)")
    _log("  D2: positive alpha [+0.6, +0.8, +1.0] (promote correctness)")
    _log("=" * 56)
    ...
    all_rows = []
    report_lines = ["IC-4-M5 D1+D2 v4 REVERSE ALPHA REPORT", "=" * 56, ""]
    ...
    _log("\n" + "=" * 56)
    _log("D1-REVERSE: CROSS-LAYER SYCOPHANCY (NEGATIVE ALPHA)")
    _log("=" * 56)

    report_lines.append("=" * 56)
    report_lines.append("D1-REVERSE: NEGATIVE ALPHA (inhibit sycophancy)")
    report_lines.append("=" * 56)
    report_lines.append(f"{'Layer':>6s} {'SNR':>8s} {'cv_acc':>8s} "
                         f"{'oracle(-0.8)':>14s} {'oracle(-1.0)':>14s} {'oracle(-1.2)':>14s} {'Δ(-1.2)':>10s}")
    ...
    for layer in D1_LAYERS:
        ...
        _log(f"  SNR={snr:.4f} probe cv_acc={probe.get('cv_acc_mean', 1.0):.4f}")

        layer_data = {"layer": layer, "snr": snr}
    ...
    for lr in layer_results:
        delta = base_sr - lr.get("oracle_a-1.2", 0)
        _log(f"  L{lr['layer']}: SNR={lr['snr']:.4f} "
             f"o(-0.8)={lr.get('oracle_a-0.8',0):.4f} "
             f"o(-1.0)={lr.get('oracle_a-1.0',0):.4f} "
             f"o(-1.2)={lr.get('oracle_a-1.2',0):.4f} Δ={delta:+.4f}")

    best = max(layer_results, key=lambda x: base_sr - x.get("oracle_a-1.2", 0))
    report_lines.append("")
    report_lines.append(f"Best oracle effect: L{best['layer']} Δ={base_sr - best.get('oracle_a-1.2', 0):+.4f}")
    _log(f"\nBest oracle effect: L{best['layer']} Δ={base_sr - best.get('oracle_a-1.2', 0):+.4f}")

    _log("\n" + "=" * 56)
    _log("D2-REVERSE: ANSWER CORRECTNESS (POSITIVE ALPHA)")
    _log("=" * 56)

    report_lines.append("")
    report_lines.append("=" * 56)
    report_lines.append("D2-REVERSE: POSITIVE ALPHA (promote correctness)")
    report_lines.append("=" * 56)
    ...
    if min(len(correct_train), len(incorrect_train)) >= 3:
        ...
        for alpha in D2_ALPHAS:
            ...
            report_lines.append(f"  α={alpha:+.1f}: oracle_cr={oracle_cr:.4f} Δ={delta_d2:+.4f} (base={d2_base_correct_rate:.4f})")
    ...
    _log("=== D1+D2 v4 (REVERSE ALPHA) DONE ===")
