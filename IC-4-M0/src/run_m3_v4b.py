...
Modes: base, prompt_only, steering_a-1.0, oracle_gate, valid_prefill_probe_gate,
       random_valid_prefill_probe_gate, shuffled_valid_prefill_probe_gate
...
def _generate_valid_prefill_probe_gated(
    model, tokenizer, test_data, steering_vector, layer_idx,
    alpha_max, probe_info, probe_cfg, gen_cfg, control_type="steering",
):
    """
    TWO-PASS PREFILL (fixes M3-v4 unsteered KV reuse bug).

    Pass 1 (NO hook): prefill forward -> hidden states -> probe -> gate -> alpha.
                      PAST_KEY_VALUES ARE DISCARDED.

    Pass 2 (WITH hook): apply steering hook at determined alpha,
                        prefill forward FROM SCRATCH -> steered KV cache,
                        -> generate tokens from steered KV cache.
    """
    ...
    for sid, sample in enumerate(test_data):
        ...
        # ============================================================
        # PASS 1: Unsteered prefill -- ONLY for probe decision.
        # KV cache from this pass is NEVER reused for generation.
        # ============================================================
        with torch.no_grad():
        ...
        # ============================================================
        # PASS 2: Steered prefill -- FROM SCRATCH with steering hook.
        # outputs_pass1.past_key_values are NOT reused.
        # ============================================================
        handle = None
        ...
        # Generate tokens from steered prefill KV cache.
        generated_ids = []
        ...
        results.append({
            **sample,
            ...
            "mode": f"valid_prefill_probe_gate_a{alpha_max}_{representation}",
            "alpha": alpha_max,
...
def _compute_verdict(df, m3v3_probe_h=0.9333):
    ...
    probe_modes = df[df["mode"].str.contains("valid_prefill_probe_gate") &
    ...
    random_p = df[df["mode"].str.contains("random_valid_prefill")]
    shuffled_p = df[df["mode"].str.contains("shuffled_valid_prefill")]
...
def _generate_report(report_path, config, df, probe_info_all, probe_preds_all, elapsed):
    ...
    for _, row in df.iterrows():
        m = str(row["mode"])
        if "valid_prefill_probe_gate" in m and "random" not in m and "shuffled" not in m:
    ...
    for seed in seeds:
        ...
        for layer_idx in layers:
            ...
            for rep in representations:
                ...
                probe_res = _generate_valid_prefill_probe_gated(
                    model, tokenizer, test, steering_v, layer_idx,
                    alpha_max, probe_info, probe_cfg_with_rep, gen_cfg, "steering",
                )
                mode_label = f"valid_prefill_probe_gate_a{alpha_max}_{rep}"
                ...
                random_res = _generate_valid_prefill_probe_gated(
                ...
                m = _evaluate_and_add(random_res, seed, layer_idx,
                                      f"random_valid_prefill_probe_gate_a{alpha_max}_{rep}",
                ...
                shuffled_res = _generate_valid_prefill_probe_gated(
                ...
                m = _evaluate_and_add(shuffled_res, seed, layer_idx,
                                      f"shuffled_valid_prefill_probe_gate_a{alpha_max}_{rep}",
