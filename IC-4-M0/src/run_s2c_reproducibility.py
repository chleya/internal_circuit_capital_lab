"""
S2c: Reproducibility verification of λ=0.15 E2 Goldilocks
2 independent seeds (42, 123), expanded test set (all available)
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_s2c_reproducibility")
...
def train_one_epoch(model, dl, v_syc, device, opt):
    ...
    for b in dl:
        ...
        loss = LAMBDA * (F.normalize(hs, p=2, dim=1) * v).sum(dim=1).mean() + o.loss
...
def run_one(seed, tokenizer, dl, v_syc, test_data, device, log_path):
    _log(f"\n  --- Seed={seed} ---", log_path)
    ...
    for ep in range(EPOCHS):
        ...
        traj.append({"epoch": ep+1, "ce": round(ce,4), "cos": round(cos,4), "mse": round(mse,4), "syc_rate": ev["syc_rate"], "quality": ev["quality"], "outputs": ev["outputs"]})
...
def main():
    ...
    _log(f"S2c: λ={LAMBDA} E2 reproducibility | seeds={SEEDS} | {time.strftime('%H:%M:%S')}", log_path)
    ...
    results = {"experiment": "S2c_Reproducibility", "lambda": LAMBDA, "epochs": EPOCHS, "baseline": b_eval, "test_size": len(test), "seeds": {}}
    t0 = time.time()
    for s in SEEDS:
        results["seeds"][str(s)] = run_one(s, tokenizer, dl, v_syc, test, device, log_path)
    elapsed = time.time() - t0

    _log(f"\n{'='*60}", log_path)
    _log(f"SUMMARY (λ={LAMBDA}, {EPOCHS}ep):", log_path)
    for s in SEEDS:
        e2 = results["seeds"][str(s)][1]
        _log(f"  Seed {s}: CE={e2['ce']:.4f} cos={e2['cos']:.4f} syc={e2['syc_rate']:.4f} qual={e2['quality']:.4f}", log_path)
