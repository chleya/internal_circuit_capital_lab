"""
S1i: Alternative Stage 1 Directional Losses — can cosine regularization or fixed-target MSE
     outperform the buggy self-referential MSE from S1g?

S1g proved temporal decoupling works, but left two open questions:
  1. The MSE metric was buggy (self-referential target → constant α²/896)
  2. Is MSE the optimal directional loss, or can we do better?

S1h showed MMD drift is WORSE than fixed steer for this problem.

This experiment tests two cleaner alternatives:
  - Cosine regularization: L = cos_sim(hs, v_syc) — direct directional optimization
  - Fixed-target MSE: target = baseline_hs + alpha * v_syc — non-self-referential MSE

Design (3 configs):
  cos_seq_3_3:     Stage1(cosine reg, 3ep, LoRA) → Stage2(CE-only, 3ep, LoRA+lm_head)
  cos_seq_5_3:     Stage1(cosine reg, 5ep, LoRA) → Stage2(CE-only, 3ep, LoRA+lm_head)
  mse_fixed_seq_3_3: Stage1(MSE with fixed baseline target, 3ep) → Stage2(CE-only, 3ep) [S1g-v2]

Fixed: r=8, lr=5e-4, 20 train samples (syc only)
Per-epoch evaluation with meaningful convergence metrics.
"""
...
RESULTS_DIR = os.path.join(BASE_DIR, "results_s1i_contrastive_stage1")
...
def compute_directional_metrics(model, dataloader, v_syc, device):
    """Compute cosine similarity and fixed-target MSE for all training samples."""
    model.eval()
...
def train_stage1_cosine(model, dataloader, v_syc, device, optimizer):
    model.train()
    total_loss = 0.0
    n_batches = 0
    v_norm = F.normalize(v_syc.unsqueeze(0).to(device), p=2, dim=1)
...
def run_config_cos_seq(base_model_cls, tokenizer, dataloader, v_syc,
                       test_data, device, log_path, n_stage1, n_stage2):
    config_label = f"cos_seq_{n_stage1}_{n_stage2}"
    _log(f"\n  === {config_label}: Stage1(cosine reg, {n_stage1}ep) → Stage2(CE, {n_stage2}ep) ===", log_path)
    _log(f"    Core test: Does direct cos_sim minimization encode directional structure effectively?", log_path)
    ...
    _log(f"    --- Stage 1: cosine regularization (lm_head frozen) ---", log_path)
    for ep in range(n_stage1):
        loss_val = train_stage1_cosine(model, dataloader, v_syc, device, opt_s1)
        model.eval()
        ...
        _log(f"      E{ep+1}: cos_loss={loss_val:.6f} | cos_sim={dir_m['cos_sim_mean']:.6f} | "
...
def run_config_mse_fixed_seq(base_model_cls, tokenizer, dataloader, v_syc,
                             test_data, device, log_path, n_stage1, n_stage2):
    config_label = f"mse_fixed_seq_{n_stage1}_{n_stage2}"
    _log(f"\n  === {config_label}: Stage1(MSE with fixed baseline target, {n_stage1}ep) → Stage2(CE, {n_stage2}ep) ===", log_path)
    _log(f"    [S1g-v2] Control: Fixed baseline target removes self-referential MSE bug", log_path)
    model = create_lora_model(base_model_cls, device, LORA_R, LORA_ALPHA)
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    _log(f"    LoRA trainable params: {trainable:,}", log_path)
    ...
    _log(f"    --- Stage 1: MSE with fixed baseline target (lm_head frozen) ---", log_path)
    for ep in range(n_stage1):
...
def main():
    ...
    _log("=" * 60, log_path)
    _log("S1i: Alternative Stage 1 Directional Losses", log_path)
    _log(f"Fixed: r={LORA_R}, alpha={ALPHA}, lr={LR}", log_path)
    _log(f"Configs: cos_seq_3_3, cos_seq_5_3, mse_fixed_seq_3_3 [S1g-v2]", log_path)
    _log(f"Key metrics: cos_sim (directional alignment), mse_fixed (non-buggy MSE)", log_path)
    ...
    dataset = S1iDataset(train_syc, tokenizer, baseline_hs, MAX_SEQ_LENGTH)
    ...
    traj = run_config_cos_seq(base_model_cls, tokenizer, dataloader, v_syc,
                              test_data, device, log_path, 3, 3)
    all_trajectories.append(traj)
    _save_partial(all_trajectories, base_eval)

    traj = run_config_cos_seq(base_model_cls, tokenizer, dataloader, v_syc,
                              test_data, device, log_path, 5, 3)
    all_trajectories.append(traj)
    _save_partial(all_trajectories, base_eval)
    ...
    final_results = {
        "experiment": "S1i_Alternative_Stage1_Losses",
        "description": (
            "Does cosine regularization or fixed-target MSE outperform the buggy self-referential "
            "MSE from S1g for encoding directional structure in Stage 1?"
        ),
        "config": {
